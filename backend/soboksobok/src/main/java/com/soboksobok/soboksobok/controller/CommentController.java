...
//        User user = (User)principal; // 현재 로그인 한 유저
        ...
        CommentResDto dto=service.createComment(Long.valueOf(1),qna_id,comment);
...
//        if(user.getUserSeq()!=user_id) return ApiResponse.fail();
    ...
    public ApiResponse updateComment(@PathVariable("comment_id") Long comment_id,@RequestParam("user_seq") Long user_seq,@RequestBody CommentReqDto dto) throws Exception{
//        org.springframework.security.core.userdetails.User principal = (org.springframework.security.core.userdetails.User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
//        Object principal = SecurityContextHolder.getContext().getAuthentication().getPrincipal();
//        User user = (User)principal; // 현재 로그인 한 유저
        // 받은 아이디와 로그인 한 유저의 아이디가 같을 때만 실행
...
//        if(user.getUserSeq()!=user_id) return ApiResponse.fail();
        if(Long.valueOf(1)!=user_seq) return ApiResponse.fail();
        CommentResDto res=service.updateComment(comment_id,user_seq,dto);
